...
function render_result(string $type, string $title, array $order = [], string $message = ''): never
{
    ...
    $courseTitle = (string) ($order['course_title'] ?? DEFAULT_SETTINGS['course_title']);
    $refId = $order['ref_id'] ?? '';
    ?>
<!doctype html>
...
<head>
  ...
  <title><?= e($title) ?></title>
  <style>
    body{margin:0;font-family:Tahoma,Arial,sans-serif;background:#f6f7fb;color:#1f2937;text-align:center;padding:40px 16px}.box{background:#fff;border-radius:20px;padding:32px;box-shadow:0 16px 40px rgba(15,23,42,.10);display:inline-block;width:min(100%,520px);text-align:right}h1{margin:0 0 16px;font-size:1.6rem;text-align:center}.success h1{color:#059669}.fail h1{color:#dc2626}.message{text-align:center;color:#4b5563;line-height:1.9}.details{margin:24px 0;padding:0;list-style:none;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden}.details li{display:flex;justify-content:space-between;gap:16px;padding:14px 16px;border-bottom:1px solid #e5e7eb}.details li:last-child{border-bottom:0}.details span{color:#6b7280}.details strong{direction:ltr;text-align:left}.actions{display:flex;flex-wrap:wrap;gap:12px;justify-content:center;margin-top:24px}.button{display:inline-block;background:#2563eb;color:#fff;text-decoration:none;padding:12px 22px;border-radius:12px;font-weight:700}.button.secondary{background:#111827}
  </style>
</head>
<body>
  <main class="box <?= e($boxClass) ?>">
    ...
    <?php if ($isSuccess): ?>
      <ul class="details"><li><span>نام دوره</span><strong><?= e($courseTitle) ?></strong></li><li><span>نام خریدار</span><strong><?= e($buyerName) ?></strong></li><li><span>مبلغ پرداخت‌شده</span><strong><?= COURSE_PRICE_TOMAN_LABEL ?></strong></li><li><span>کد پیگیری سفارش</span><strong><?= e($trackingCode) ?></strong></li><li><span>شماره پیگیری پرداخت</span><strong><?= e((string) $refId) ?></strong></li></ul>
    <?php else: ?>
      <p class="message">پرداخت انجام نشد یا توسط کاربر لغو شد.</p>
      <?php if ($trackingCode !== ''): ?><ul class="details"><li><span>کد پیگیری سفارش</span><strong><?= e($trackingCode) ?></strong></li></ul><?php endif; ?>
    <?php endif; ?>
    <div class="actions"><?php if (!$isSuccess): ?><a class="button" href="<?= e(SITE_URL) ?>#start">تلاش دوباره</a><?php endif; ?><a class="button secondary" href="<?= e(SITE_URL) ?>">بازگشت به سایت</a></div>
  </main>
</body>
</html>
    <?php exit;
...
if ($authority === '') render_result('fail', 'خطا در بازگشت از درگاه', [], 'شناسه پرداخت از زرین‌پال دریافت نشد.');
$orders = read_orders();
$orderIndex = find_order_index_by_authority($orders, $authority);
if ($orderIndex < 0) render_result('fail', 'سفارش پیدا نشد', [], 'سفارشی با این شناسه پرداخت در سیستم ثبت نشده است.');
$order = $orders[$orderIndex];
if (($order['status'] ?? '') === 'paid') render_result('success', 'این سفارش قبلاً پرداخت شده است', $order, 'پرداخت این سفارش پیش‌تر با موفقیت ثبت شده است.');
...
if ($code === 100 || $code === 101) {
    ...
    write_orders($orders);
    mark_lead_converted((string) ($orders[$orderIndex]['phone'] ?? ''));
