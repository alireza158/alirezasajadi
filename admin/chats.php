...
<section class="admin-card">
  ...
  <div class="table-responsive"><table class="table align-middle admin-table"><thead><tr><th>session_id</th><th>نام کاربر</th><th>موبایل</th><th>آخرین پیام کاربر</th><th>تعداد پیام‌ها</th><th>شروع</th><th>آخرین پیام</th><th>وضعیت</th><th>مشاهده</th></tr></thead><tbody>
  <?php foreach ($filtered as $i => $chat): $modalId = 'chatModal' . $i; $messages = is_array($chat['messages'] ?? null) ? $chat['messages'] : []; ?>
    <tr><td class="ltr small text-nowrap"><?= e($chat['session_id'] ?? '-') ?></td><td><?= e($chat['user_name'] ?? '-') ?></td><td class="ltr text-nowrap"><?= e($chat['user_phone'] ?? '-') ?></td><td class="wide-cell"><?= e(mb_substr(extract_last_user_message($chat), 0, 90, 'UTF-8')) ?></td><td><?= e(count($messages)) ?></td><td class="ltr text-nowrap"><?= e(format_datetime($chat['started_at'] ?? '')) ?></td><td class="ltr text-nowrap"><?= e(format_datetime($chat['last_message_at'] ?? '')) ?></td><td><?= render_status_badge((string)($chat['status'] ?? 'open')) ?></td><td><button class="btn btn-sm btn-outline-primary" type="button" data-bs-toggle="modal" data-bs-target="#<?= e($modalId) ?>">مشاهده گفتگو</button></td></tr>
